...
const apiHandlers = [
  http.post<{ hmsnr: string }, OppslagRequest, OppslagResponse>(
    `${DELBESTILLING_API_PATH}/hjelpemidler/:hmsnr/deler`,
    async ({ request, params }) => {
      ...
      // For lokal testing uten kall mot backend
      if (hmsnr === '999999') {
        return HttpResponse.json({
          hjelpemiddel: {
            ...
            erInnenforGaranti: false,
            antallÅrGaranti: false,
          },
          feil: undefined,
          piloter: [],
        })
  ...
  http.get<{ hmsnr: string }, {}, OppslagResponseUtenDeler>(
    `${DELBESTILLING_PUBLIC_API_PATH}/hjelpemidler/:hmsnr`,
    async ({ params }) => {
      const hmsnr = params['hmsnr']
  ...
  http.post<{}, DelbestillingRequest, DelbestillingResponse>(
    `${DELBESTILLING_API_PATH}/delbestilling`,
    async ({ request }) => {
